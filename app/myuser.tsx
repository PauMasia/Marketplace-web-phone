...
export default function UserDetail() {
    ...
    useEffect(() => {
        AsyncStorage.getItem("partner_id").then(stored => {
            if (!stored) return;
    ...
    useEffect(() => {
        fetch(`${apiUrl}/userList/${partnerId}`)
            .then(res => res.json())
            .then(data => {
    ...
    // de forma que asi no se intenta cargas hasta que esta acabado de cargar el useer
    if (!user) {
        return (
    ...
    const deleteUser = () => {
        ...
        fetch(`${apiUrl}/userList/deluser/${partnerId}`, { method: "DELETE" })
            .then((res) => {
                if (res.ok) {
                    if (typeof window !== "undefined") {
